...
<div class="row clearfix mt20 yahei black6 box basicInfo" style="width: 960px;padding:20px;background-color: #FFF;">
    ...
    <div class="boxShow">
        <div class="item name">
            <label>姓名</label><span>{$worker->name}</span>
        </div>
        {if $worker->email}
            <div class="item email">
                <label>邮箱</label><span>{$worker->email}</span>
            </div>
        {/if}
        {if $worker->regionid}
            <div class="item region">
                <label>地区</label>
                <span>
                    {$worker->ancestornames}{$worker->regionname}
                </span>
            </div>
        {/if}
        {if $worker->tall}
            <div class="item tall">
                <label>身高</label><span>{$worker->tall}cm</span>
            </div>
        {/if}
        {if $worker->weight}
            <div class="item weight">
            <label>体重</label><span>{$worker->weight}kg</span>
        </div>
        {/if}
        {if $worker->sex}
            <div class="item sex">
                <label>性别</label><span>{if $worker->sex == 1}男{else}女{/if}</span>
            </div>
        {/if}
        {if $worker->bust||$worker->waist||$worker->hip}
            <div class="item bwh">
                <label>三围</label>
                {if $worker->bust&&$worker->bust>0}<span>{$worker->bust}(胸围)</span>{/if}
                {if $worker->waist&&$worker->waist>0}<span class="ml10">{$worker->waist}(腰围)</span>{/if}
                {if $worker->hip&&$worker->hip>0}<span class="hip ml10">{$worker->hip}(臀围)</span>{/if}
            </div>
        {/if}
        {if $worker->cup&&$worker->sex==0}
            <div class="item cup">
                <label>罩杯</label>
                <span>{$worker->cup}</span>
            </div>
        {/if}
        {if $worker->hair}
            <div class="item hair">
                <label>发色</label><span>{$worker->hairname}</span>
            </div>
        {/if}
        {if $worker->shoe}
            <div class="item shoe">
                <label>鞋号</label><span>{$worker->shoe}</span>
            </div>
        {/if}
        {if $worker->birthday&&$worker->birthday!='0000-00-00'}
            <div class="item birthday">
                <label>生日</label><span>{$worker->birthday}</span>
            </div>
        {/if}
        {if $worker->constellationname}
            <div class="item constellation">
                <label>星座</label><span>{$worker->constellationname}</span>
            </div>
        {/if}
        {if $worker->degree}
            <div class="item degree">
                <label>学历</label><span>{$worker->degreename}</span>
            </div>
        {/if}
        {if $worker->profession}
            <div class="item profession">
                <label>专业</label><span>{$worker->professionname}</span>
            </div>
        {/if}
        {if $worker->hobby}
            <div class="item hobby">
                <label>爱好特长</label><span>{$worker->hobby}</span>
            </div>
        {/if}
        {if $worker->regions}
            <div class="item">
