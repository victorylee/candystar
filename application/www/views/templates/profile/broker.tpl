...
<div class="row clearfix mt20 yahei black6 topBox" style="width: 1000px;background-color: #FFF;">
    <div class="boxShow" style="width: 1000px;height: 125px;">
        <div class="col" style="width: 150px;padding: 10px;position: relative;">
            {assign var=dcd value=$broker->dcd|json_decode:true}
        ...
        <div class="col" style="width: 530px;">
            <p class="fs20 mt30" style="color: #3879D9;">{$broker->name}</p>
            <p class="mt10 region">
                <span provinceid="{$broker->pid}" cityid="{$broker->regionid}">
                {if $broker->regionid}{$broker->ancestornames}{$broker->regionname}{/if}
                </span>
            </p>
            <p class="mt10 business">
                {foreach from=$broker->business item=item}
                    <span rel="{$item->business}" class="mr10">{$item->businessname}</span>
                {/foreach}
            </p>
        </div>
    </div>
...
<div class="row clearfix mt20 yahei black6 box basicInfo" style="width: 960px;padding:20px;background-color: #FFF;">
    ...
    <div class="boxShow">
        <div class="item name">
            <label>姓名</label><span rel="{$broker->name}">{$broker->name}</span>
        </div>
        <div class="item email">
            <label>邮箱</label><span rel="{$broker->email}">{if $broker->email}{$broker->email}{else}未填写{/if}</span>
        </div>
        <div class="item region">
            <label>地区</label>
            <span provinceid="{$broker->pid}" cityid="{$broker->regionid}">
                {if $broker->regionid}{$broker->ancestornames}{$broker->regionname}{else}未填写{/if}
            </span>
        </div>
    </div>
    <div class="boxHidden" style="display: none;">
        <form id="topForm" method="post">
            ...
            <div class="item" style="width:1000px;">
                ...
                <span class="error" style="top:122px;"></span>
                <div class="businessDemo" style="display:inline-block;width:300px;height: 80px;border: 1px solid #CCC;margin-left: 110px;">
...
<div class="row clearfix mt20 yahei black6 expBox" style="width: 960px;padding:20px;background-color: #FFF;">
    ...
    <div class="exps">
        {if $broker->cases}
            ...
            {foreach from=$broker->cases item=item}
...
<div class="row clearfix mt20 yahei black6 jobBox" style="width: 960px;padding:20px;background-color: #FFF;">
    ...
    <div class="jobs">
        {if $broker->jobs}
            ...
            {foreach from=$broker->jobs item=item}
                <ul class="job{$item->id}">
                    <li class="date">从<span class="start">{$item->startdate}</span>到<span class="end">{$item->enddate}</span></li>
                    <li class="name">{$item->name}</li>
                    <li class="skill" rel="{$item->skill}">{$item->skillname}</li>
                    <li class="sex" rel="{$item->sex}">{$item->sexname}</li>
                    <li class="quantity">{$item->quantity}</li>
                    <li class="salary">{if $item->salary}{$item->salary}{else}面议{/if}</li>
    ...
    <div class="boxHidden" style="width: 600px;margin-left: 100px;display: none;">
        <form>
            ...
            <div class="item">
                <label>性别：</label>
                <select name="sex">
                    {foreach from=$sexList item=item}
                        <option value="{$item->id}">{$item->name}</option>
                    {/foreach}
                </select>
                <span class="error"></span>
            ...
            <div class="item">
                <label>日薪：</label><input type="text" name="salary"/><label for="salary" style="width:60px;"><input type="checkbox" id="salary"/>面议</label><span class="error"></span>
            </div>
            <div class="item">
                <label style="vertical-align: top;">描述：</label><textarea name="description" style="width:305px;height:100px;resize: none;"></textarea><span class="error"></span>
            </div>
...
//            }
        ...
        $(".jobBox .edit").live("click",function(){
            ...
            var salary = $("ul[class='job"+id+"']").find('.salary').text();
            if(salary == 0||salary == ''||salary == '面议'){
                $("#salary").attr("checked",true);
                $(".jobBox .boxHidden input[name='salary']").attr("readonly",true);
                $(".jobBox .boxHidden input[name='salary']").val('');
            }else{
                $("#salary").attr("checked",false);
                $(".jobBox .boxHidden input[name='salary']").attr("readonly",false);
                $(".jobBox .boxHidden input[name='salary']").val(salary);
            }
        ...
        $("#salary").live("click",function(){
            var ischecked = $(this).attr("checked");
            if(ischecked == 'checked'){
                $(".jobBox input[name='salary']").val("");
                $(".jobBox input[name='salary']").attr("readonly",true);
            }else{
                $(".jobBox input[name='salary']").attr("readonly",false);
            }
        });
        //保存工作经历
        var submited=false;
...
//            }
            ...
            $.post("/profile/saveBrokerJob",$(".jobBox form").serializeObject(),function(data){
                ...
                if(data.success){ //成功
                    ...
                    console.log($("ul.job"+data.id).html())
        ...
        $(".jobBox .cancel").live("click",function(){
            ...
            $(".jobBox form")[0].reset();
            $(".jobBox input[name='id']").val('');
            $(".jobBox input[name='salary']").attr("readonly",false);
            $(".jobBox #salary").attr("checked",false);
        });
    });
</script>
</html>
