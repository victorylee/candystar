...
<div class="row clearfix mt20 yahei black6 topBox" style="width: 1000px;background-color: #FFF;">
    <div class="boxShow" style="width: 1000px;height: 125px;">
        <div class="col" style="width: 150px;padding: 10px;position: relative;">
            {assign var=dcd value=$worker->dcd|json_decode:true}
            <img src="{if isset($dcd.avatar)&&$dcd.avatar}{$dcd.avatar}{else}/images/avatarBig.png{/if}" class="userAvatar" style="width: 120px;"/>
        ...
        <div class="col" style="width: 530px;">
            <p class="fs20 mt30" style="color: #3879D9;">{$worker->name}<span class="ml10 fs14 sex">{if $worker->sex == 1}男{elseif $worker->sex == 0}女{/if}</span></p>
            <p class="mt10 region">
                <span provinceid="{$worker->pid}" cityid="{$worker->regionid}">
                {if $worker->regionid}{$worker->ancestornames}{$worker->regionname}{/if}
                </span>
            </p>
            <p class="mt10">
                {foreach from=$worker->skills item=item}
                    <span rel="{$item->skill}" class="mr10">{$item->name}</span>
                {/foreach}
            </p>
        </div>
    </div>
        ...
        <div style="position: relative;">
            ...
            <div class="clearfix" style="padding-bottom:15px;">
                ...
                <div class="avatars clearfix fl" style="display:inline-block;width:450px;height: 300px;">
                    ...
                    {assign var=dcd value=$worker->dcd|json_decode:true}
                    <div class="big item_avatar" style="margin-top: 20px;"><img src="{if isset($dcd.avatar)&&$dcd.avatar}{$dcd.avatar}{else}/images/avatarBig.png{/if}" width="256" height="256"></div>
                </div>
            </div>
...
<div class="row clearfix mt20 yahei black6 box snapshotBox" style="width: 960px;padding:20px;background-color: #FFF;">
    <h3 class="mb20">个人照片<a href="javascript:void(0);" class="fr fs14 fwn mr10 edit">编辑</a></h3>
    <div class="boxShow">
        <div id="playimages" class="play clearfix relative">
            <div style="width: 635px; margin: auto;" class="snapshots">
                {assign var=dcd value=$worker->dcd|json_decode:true}
                {if (isset($dcd.imgs)&& $dcd.imgs) }
                    <div style="width: 635px; margin: auto;">
                        <div id="showcase" class="showcase">
                            <!-- 图片 start -->
                            {if isset($dcd.imgs)}
                                {foreach from=$dcd.imgs  item="item"}
                                    <div class="showcase-slide">
                                        <div class="showcase-thumbnail">
                                            <img src="{$item}"  name='resize'/>
    ...
    <div class="col clearfix relative pb30 boxHidden" style="width:900px;display: none;">
        <div>
            <div class="clearfix ml15">
                <p class="fr black6 fs12 lh16" style="display:inline-block;width: 500px;text-align: left;margin-right: 130px;">
                    请尽量上传正面、左侧面、右侧面、背面、近身照、全景照<br/>
                    请至少上传3张产品截图，图片最佳尺寸为800*500px<br/>
                    图片文件大小不超过2MB，支持JPG、JPEG、GIF、PNG文件
                </p>
                <input class="fl rc5 fwb fs14 ml15" type="file" id="pic_upload" value="批量上传图片" style="width:200px;height:52px;background:#f5f5f5;border:solid 1px #ddd;border-top:none;box-shadow: 0px -2px 2px #ddd inset;">
            </div>
            <div class="mt15 imglist clearfix ml15 pt15 mr15"  style="border-top:1px solid #CCC;">
                <ul id="picList" class="clearfix" style="height:auto;">
                    {assign var=dcd value=$worker->dcd|json_decode:true}
                    {if isset($dcd.imgs)}
                        {foreach from=$dcd.imgs item="item"}
                            <li rel="">
            ...
            <div class="ml15">
                <a href="javascript:void(0);" class="button save mt20" style="margin-left: 300px;">确定</a>
                <a href="javascript:void(0);" class="button cancel">取消</a>
            </div>
        </div>
    </div>
...
<div class="row clearfix mt20 yahei black6 box basicInfo" style="width: 960px;padding:20px;background-color: #FFF;">
    ...
    <div class="boxShow">
        <div class="item name">
            <label>姓名</label><span rel="{$worker->name}">{$worker->name}</span>
        </div>
        <div class="item email">
            <label>邮箱</label><span rel="{$worker->email}">{if $worker->email}{$worker->email}{else}未填写{/if}</span>
        </div>
        <div class="item region">
            <label>地区</label>
            <span provinceid="{$worker->pid}" cityid="{$worker->regionid}">
                {if $worker->regionid}{$worker->ancestornames}{$worker->regionname}{else}未填写{/if}
            </span>
        </div>
        <div class="item tall">
            <label>身高</label><span rel="{$worker->tall}">{if $worker->tall}{$worker->tall}cm{else}未填写{/if}</span>
        </div>
        <div class="item weight">
            <label>体重</label><span rel="{$worker->weight}">{if $worker->weight}{$worker->weight}kg{else}未填写{/if}</span>
        </div>
        <div class="item sex">
            <label>性别</label><span rel="{$worker->sex}">{if $worker->sex == 1}男{else}女{/if}</span>
        </div>
        <div class="item bwh">
            <label>三围</label><span class="bust" rel="{$worker->bust}">{if $worker->bust}{$worker->bust}{else}未填写{/if}</span><span class="waist ml10" rel="{$worker->waist}">{if $worker->waist}{$worker->waist}{else}未填写{/if}</span><span class="hip ml10" rel="{$worker->hip}">{if $worker->hip}{$worker->hip}{else}未填写{/if}</span>
        </div>
        <div class="item cup">
            <label>罩杯</label><span rel="{$worker->cup}">{$worker->cup}</span>
        </div>
        <div class="item hair">
            <label>发色</label><span rel="{$worker->hair}">{$worker->hairname}</span>
        </div>
        <div class="item shoe">
            <label>鞋号</label><span rel="{$worker->shoe}">{if $worker->shoe}{$worker->shoe}{else}未填写{/if}</span>
        </div>
        <div class="item birthday">
            <label>生日</label><span rel="{if $worker->birthday!='0000-00-00'}{$worker->birthday}{/if}">{if $worker->birthday&&$worker->birthday!='0000-00-00'}{$worker->birthday}{else}未填写{/if}</span>
        </div>
        <div class="item constellation">
            <label>星座</label><span rel="{$worker->constellation}">{if $worker->constellationname}{$worker->constellationname}{else}未填写{/if}</span>
        </div>
        <div class="item degree">
            <label>学历</label><span rel="{$worker->degree}">{if $worker->degree}{$worker->degreename}{else}未填写{/if}</span>
        </div>
        <div class="item profession">
            <label>专业</label><span rel="{$worker->profession}">{if $worker->profession}{$worker->professionname}{else}未填写{/if}</span>
        </div>
        <div class="item hobby">
            <label>爱好特长</label><span rel="{$worker->hobby}">{if $worker->hobby}{$worker->hobby}{else}未填写{/if}</span>
        </div>
        <div class="item">
            <label>工作地点</label>
            <div class="workcitys" style="display: inline-block;">
                {foreach from=$worker->regions item=item}
                    <span rel="{$item->regionid}">{$item->regionname}</span>
                {/foreach}
            </div>
        </div>
        <div class="item">
    ...
    <div class="boxHidden" style="display: none;">
        <form id="topForm" method="post">
            ...
            <div class="item">
                <label>胸围</label><input type="text" name="bust"/><span class="error"></span>
            </div>
            <div class="item">
                <label>腰围</label><input type="text" name="waist"/><span class="error"></span>
            </div>
            <div class="item">
                <label>臀围</label><input type="text" name="hip"/><span class="error"></span>
            </div>
            <div class="item">
                <label>罩杯</label><input type="text" name="cup"/><span class="error"></span>
            </div>
            <div class="item">
                <label>发色</label>
                <select name="hair">
                    {foreach from=$hairList item=item}
                        <option value="{$item->id}">{$item->name}</option>
                    {/foreach}
                </select>
                <span class="error"></span>
            ...
            <div class="item">
                <label>星座</label><select name="constellation">
                    {foreach from=$constellationList item=item}
                        <option value="{$item->id}">{$item->name}</option>
                    {/foreach}
            ...
            <div class="item">
                <label>学历</label><select name="degree">
                    {foreach from=$degreeList item=item}
                        <option value="{$item->id}">{$item->name}</option>
                    {/foreach}
                </select>
                <label>专业</label><select name="profession">
                    <option value="">请选择</option>
                    {foreach from=$professionList item=item}
                        <option value="{$item->id}">{$item->name}</option>
                    {/foreach}
                </select>
                <span class="error"></span>
            ...
            <div class="item" style="width:1000px;">
                ...
                <span class="error"></span>
                <div class="workcityDemo" style="display:inline-block;width:300px;height: 80px;border: 1px solid #CCC;margin-left: 113px;">
            ...
            <div class="item" style="width:1000px;margin-top: 10px;">
                <label style="vertical-align: top;float: left;">行业标签</label>
                <div class="businessBox fl" style="display:inline-block;width:300px;height: 80px;border: 1px solid #CCC;"></div>
                <span class="error"></span>
                <div class="businessDemo" style="display:inline-block;width:300px;height: 135px;border: 1px solid #CCC;margin-left: 113px;">
                    {foreach from=$skillList item=item}
                        <a rel="{$item->id}" href="javascript:void(0);">{$item->name}</a>
                    {/foreach}
                </div>
            </div>
            <div class="item mt10">
...
<div class="row clearfix mt20 yahei black6 expBox" style="width: 960px;padding:20px;background-color: #FFF;">
    ...
    <div class="exps">
        <ul><li class="date fs16">时间</li><li class="result fs16">作品</li><li class="skill fs16">角色</li><li class="description fs16">描述</li><li class="operation fs16">操作</li></ul>
        {foreach from=$worker->exps item=item}
            <ul class="exp{$item->id}">
                <li class="date">从<span class="start">{$item->startmonth|date_format:"Y-m"}</span>到<span class="end">{$item->endmonth|date_format:"Y-m"}</span></li>
                {assign var=dcd value=$item->dcd|json_decode:true}
                <li class="result">{$dcd.result}</li>
                <li class="skill" rel="{$item->skill}">{$item->skillname}</li>
                <li class="description">{$dcd.description}</li>
                <li class="operation"><a href="javascript:void(0);" class="edit fs12" rel="{$item->id}">修改</a><a href="javascript:void(0);" class="delete fs12 ml10" rel="{$item->id}">删除</a> </li>
            </ul>
        {/foreach}
    </div>
...
<script type="text/javascript">
    ...
    function setSelect(num,direction,data){
        ...
        if (direction == 'top'){
            x1 = 78;
            y1 = (num > 256) ? (num - 120)/2 : 0;
        }else if(direction == 'left'){
            x1 = (num > 256) ? (num - 120)/2 : 0;
            y1 = 78;
...
//        });
    ...
    $(document).ready(function(){
        ...
        $(".basicInfo .edit").live("click",function(){
            ...
            $(".basicInfo input[name='cup']").val($(".basicInfo .cup span").attr("rel"));
        ...
        $(".basicInfo .save").live("click",function(){
            ...
            $.post("/profile/saveWorkerBasicInfo",$.extend(true,$(".basicInfo form").serializeObject(),{ business:business,workcitys:workcitys}),function(data){
                ...
                if(parseInt(data) == 1){ //成功
                    ...
                    $(".basicInfo .tall span").text($(".basicInfo input[name='tall']").val());
                    ...
                    $(".basicInfo .bust").text($(".basicInfo input[name='bust']").val());
                    $(".basicInfo .waist").text($(".basicInfo input[name='waist']").val());
                    $(".basicInfo .hip").text($(".basicInfo input[name='hip']").val());
                    $(".basicInfo .cup span").text($(".basicInfo input[name='cup']").val());
