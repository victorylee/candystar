...
!function( $ ) {
	...
	var Datepicker = function(element, options){
        ...
        this.minviewMode = options.minviewMode||parseInt(this.element.attr('data-date-minviewMode'))||0;
		this.viewMode = options.viewMode||parseInt(this.element.attr('data-date-viewMode'))||0;
	...
	Datepicker.prototype = {
		...
		fill: function() {
			...
			while(prevMonth.valueOf() < nextMonth) {
				...
				clsName = '';
				if (prevMonth.getMonth() != month && prevMonth < d) {
					clsName += ' old';
				} else if (prevMonth.getMonth() != month && prevMonth>d) {
					clsName += ' new';
				}
					...
					case 'span':
						if (target.is('.month')) {
                            ...
                            if(this.minviewMode==1){
                                ...
                                this.date = new Date(year, month, 15,0,0,0,0);
                                this.viewDate = new Date(year, month, 15,0,0,0,0);
                                this.setValue();
                                this.hide();
                            }else{
                                this.showMode(-1);
                            ...
                            if(this.minviewMode==2){
                                var year = this.viewDate.getFullYear();
                                this.date = new Date(year, 1, 15,0,0,0,0);
                                this.viewDate = new Date(year, 1, 15,0,0,0,0);
                                this.setValue();
                                this.hide();
                            }else{
                                this.showMode(-1);
